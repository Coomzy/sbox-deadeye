...
public class Player : Component
{
	...
	TimeSince timeSinceStartedDecisionMaking { get; set; }
	...
	void WalkingUpdate()
	{
		...
		var newPos = MoveTowards(Transform.Position, targetPos, 200.0f * Time.Delta);
	...
	async void ExecuteCommands()
	{
		foreach (var target in targets)
		{
			...
			if (target.isBadTarget)
			{
				Sandbox.Services.Stats.Increment("bad-guys-killed", 1);
			}
			else
			{
